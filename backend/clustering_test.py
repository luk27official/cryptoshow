...
# Filter points with scores > 0.75
high_score_points = points[points[:, 3] > 0.75][:, :3]  # Extract only (x, y, z) coordinates
...
ax.set_ylabel("Y")
ax.set_zlabel("Z")
ax.set_title("3D Clustering with DBSCAN (Filtered Scores > 0.75)")
ax.legend()
plt.show()
